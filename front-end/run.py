...
def show(bucket_name):
    ...
    for obj in objects:
        if obj['name'].split(".")[-1].lower() in ("mp4", "mov", "avi"):
            listobj.append((obj['name'], bucket_name, f"http://178.128.21.41:8000/makegif"))
    
    return render_template("show.html", display=bucket_name+"/display", bucketname=bucket_name, listrender=listobj, make_all=f"http://178.128.21.41:8000/makegif")
